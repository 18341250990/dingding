...
from base.report_config import inAndOutStockFlow_path,report_body
...
class InAndOutStockFlow(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.path = inAndOutStockFlow_path
    ...
    def test_inAndOutStockGather(self):
        ...
        if res.status_code == 200 and ('出入库业务流水' in res.text):
            etime = time.time()
            self.log.logger.info(res.text)
            self.log.logger.info(f'出入库业务流水查询耗时:{etime-stime} s')
        else:
