...
from base.report_config import returnmedicinecount_path,report_body
...
class Returnmedicinecount(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.path = returnmedicinecount_path
    ...
    def test_returnmedicinecount(self):
        self.report['type'] = 1
        ...
        if res.status_code == 200 and ('退货汇总' in res.text) :
            etime = time.time()
            self.log.logger.info(res.text)
            self.log.logger.info(f'门诊退药汇总表查询耗时:{etime-stime} s')
        else:
