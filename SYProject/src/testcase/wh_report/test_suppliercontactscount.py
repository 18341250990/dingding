...
from base.report_config import supplier_report_body,suppliercontactscount_path
...
class Suppliercontactscount(unittest.TestCase):

    def setUp(self,log=None):
        if not log:
            self.log = Logger()
        else:
            self.log = log
        ...
        self.path = suppliercontactscount_path
        self.report = supplier_report_body
    ...
    def test_suppliercontactscount(self):
        self.report['type'] = 1
        ...
        if res.status_code == 200 and ('供应商往来汇总' in res.text):
            etime = time.time()
            self.log.logger.info(res.text)
            self.log.logger.info(f'供应商往来汇总表查询耗时:{etime-stime} s')
        else:
