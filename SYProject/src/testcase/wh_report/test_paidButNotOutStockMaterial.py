...
from base.report_config import paidButNotOutStockMaterial_path,report_body
...
class PaidButNotOutStockMaterial(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.path = paidButNotOutStockMaterial_path
    ...
    def test_paidButNotOutStockMaterial(self):
        self.report['type'] = 1
        ...
        if res.status_code == 200 and ('已收费未发货' in res.text):
            etime = time.time()
            self.log.logger.info(res.text)
            self.log.logger.info(f'已收费未发货查询耗时:{etime-stime} s')
        else:
