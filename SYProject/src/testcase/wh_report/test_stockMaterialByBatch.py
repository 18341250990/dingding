...
from base.report_config import stockMaterialByBatch_path,report_body
...
class StockMaterialByBatch(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.path = stockMaterialByBatch_path
    ...
    def test_stockMaterialByBatch(self):
        self.report['type'] = 1
        ...
        if res.status_code == 200 and ('在库物品批次' in res.text):
            etime = time.time()
            self.log.logger.info(res.text)
            self.log.logger.info(f'在库物品批次查询:{etime-stime} s')
        else:
