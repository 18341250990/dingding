...
from base.report_config import instockdetail_path,report_body
...
class Instockdetail(unittest.TestCase):
    ...
    def test_instockdetail(self):
        ...
        if res.status_code == 200 and ('采购入库明细' in res.text):
            etime = time.time()
            self.log.logger.info(res.text)
            self.log.logger.info(f'采购入库明细:{etime-stime} s')
        else:
