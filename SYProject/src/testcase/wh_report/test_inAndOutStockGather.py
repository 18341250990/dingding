...
from base.report_config import inAndOutStockGather_path,report_body
...
class InAndOutStockGather(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.path = inAndOutStockGather_path
    ...
    def test_inAndOutStockGather(self):
        self.report['type'] = 1
        ...
        if res.status_code == 200 and ('出入库业务汇总' in res.text):
            etime = time.time()
            self.log.logger.info(res.text)
            self.log.logger.info(f'出入库业务汇总查询耗时:{etime-stime} s')
        else:
