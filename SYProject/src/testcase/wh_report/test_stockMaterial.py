...
from base.report_config import stockMaterial_path,report_body
...
class StockMaterial(unittest.TestCase):
    ...
    def test_stockMaterial(self):
        ...
        if res.status_code == 200 and ('在库物品' in res.text):
            etime = time.time()
            self.log.logger.info(res.text)
            self.log.logger.info(f'在库物品:{etime-stime} s')
        else:
