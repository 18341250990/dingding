...
from base.report_config import salesreturncount_path,report_body
...
class Salesreturncount(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.path = salesreturncount_path
    ...
    def test_salesreturncount(self):
        self.report['type'] = 1
        ...
        if res.status_code == 200 and ('采购退货汇总' in res.text):
            etime = time.time()
            self.log.logger.info(res.text)
            self.log.logger.info(f'采购退货汇总:{etime-stime} s')
        else:
