...
from base.report_config import instockcount_path,report_body
...
class Instockcount(unittest.TestCase):
    ...
    def test_instockcount(self):
        ...
        if res.status_code == 200 and ('采购入库汇总' in res.text):
            etime = time.time()
            self.log.logger.info(res.text)
            self.log.logger.info(f'采购入库汇总:{etime-stime} s')
        else:
