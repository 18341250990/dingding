...
from base.report_config import salesreturndetail_path,report_body
...
class Salesreturndetail(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.path = salesreturndetail_path
    ...
    def test_salesreturndetail(self):
        self.report['type'] = 1
        ...
        if res.status_code == 200 and ('采购退货明细' in res.text):
            etime = time.time()
            self.log.logger.info(res.text)
            self.log.logger.info(f'采购退货明细:{etime-stime} s')
        else:
