...
from base.report_config import dispensemedicinedetail_path,report_body
...
class Dispensemedicinedetail(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.path = dispensemedicinedetail_path
    ...
    def test_dispensemedicinedetail(self):
        self.report['type'] = 1
        ...
        if res.status_code == 200 and ('发货明细' in res.text):
            etime = time.time()
            self.log.logger.info(f'门诊发药明细表查询耗时:{etime-stime} s')
