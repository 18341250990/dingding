...
from base.report_config import dispensemedicinecount_path,report_body
...
class Dispensemedicinecount(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.path = dispensemedicinecount_path
    ...
    def test_dispensemedicinecount(self):
        self.report['type'] = 1
        ...
        if res.status_code == 200 and ('发货汇总' in res.text):
            etime = time.time()
            self.log.logger.info(res.text)
            self.log.logger.info(f'门诊发药汇总表查询耗时:{etime-stime} s')
        else:
