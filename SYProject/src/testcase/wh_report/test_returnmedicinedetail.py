...
from base.report_config import returnmedicinedetail_path,report_body
...
class Returnmedicinedetail(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.path = returnmedicinedetail_path
    ...
    def test_returnmedicinedetail(self):
        self.report['type'] = 1
        ...
        if res.status_code == 200 and ('退货明细' in res.text) :
            etime = time.time()
            self.log.logger.info(res.text)
            self.log.logger.info(f'门诊退药明细表查询耗时:{etime-stime} s')
        else:
